...
// MARK: - Saturn Return Alert Banner
// ====================================
// Prominent banner shown when portfolio stocks approach Saturn Return

struct SaturnReturnAlertBanner: View {
    let alert: SaturnReturnAlert
    var onTap: (() -> Void)? = nil
    ...
    var body: some View {
        Button(action: { onTap?() }) {
            HStack(spacing: 14) {
                ...
                ZStack {
                    Circle()
                        .fill(alert.urgency.color.opacity(0.2))
                        .frame(width: 44, height: 44)

                    Image(systemName: "circle.hexagongrid.fill")
                        .font(.title2)
                        .foregroundColor(alert.urgency.color)
                }
                ...
                VStack(alignment: .leading, spacing: 4) {
                    HStack(spacing: 6) {
                        ...
                        if alert.data.isInSaturnReturn {
                            Text("ACTIVE")
            ...
            .background(
                RoundedRectangle(cornerRadius: 16)
                    ...
                    .overlay(
                        RoundedRectangle(cornerRadius: 16)
                            .stroke(alert.urgency.color.opacity(0.3), lineWidth: 1)
                    )
            )
        }
        .buttonStyle(.plain)
    ...
    private var timeUntilText: String {
        if alert.data.isInSaturnReturn {
            return "Currently in transformation period"
        } else if alert.daysUntil < 30 {
            return "In \(alert.daysUntil) days"
        } else {
            let months = alert.daysUntil / 30
...
// MARK: - Saturn Return List Section
// ====================================
// Section for portfolio view showing approaching Saturn Returns

struct SaturnReturnListSection: View {
    let stocks: [Stock]
    @State private var saturnService = SaturnReturnService.shared
    @State private var selectedAlert: SaturnReturnAlert?

    var alerts: [SaturnReturnAlert] {
        saturnService.getApproachingSaturnReturns(from: stocks)
    }
    ...
    var body: some View {
        if !alerts.isEmpty {
            ...
            .sheet(item: $selectedAlert) { alert in
                SaturnReturnDetailSheet(stock: alert.stock, data: alert.data)
            }
        }
    }
}

// MARK: - Compact Saturn Return Badge
// ====================================
// Small badge for stock rows

struct SaturnReturnBadge: View {
    let stock: Stock
    @State private var saturnService = SaturnReturnService.shared

    var body: some View {
        if let data = saturnService.getSaturnReturn(for: stock),
           data.status == .approaching || data.status == .inSaturnReturn {
            HStack(spacing: 4) {
                Image(systemName: "circle.hexagongrid.fill")
                    .font(.system(size: 8))

                if data.isInSaturnReturn {
                    Text("SR")
                        .font(.system(size: 8, weight: .bold, design: .monospaced))
                }
            }
            ...
            .background(
                Capsule()
                    .fill(data.status.color.opacity(0.2))
            )
        }
    }
}
...
#Preview("Alert Banner") {
    ...
    let data = SaturnReturnData(
        ...
        companyAgeYears: 48.7,
        currentSaturnReturnNumber: 2,
        nextSaturnReturnDate: Calendar.current.date(byAdding: .month, value: 3, to: Date())!,
        previousSaturnReturnDate: Calendar.current.date(from: DateComponents(year: 2005, month: 4, day: 1)),
        status: .approaching
    )
    let alert = SaturnReturnAlert(
        stock: stock,
        data: data,
        headline: "AAPL approaches Saturn Return",
        message: "Apple will hit its Saturn Return in ~3 months.",
        daysUntil: 90,
        urgency: .medium
    )

    SaturnReturnAlertBanner(alert: alert)
        .padding()
